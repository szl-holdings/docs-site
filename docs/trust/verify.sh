...
#        (from the repository root: bash docs/trust/verify.sh)
#
...
TRUST_BASE="${SZL_TRUST_BASE:-https://raw.githubusercontent.com/szl-holdings/docs-site/main/docs/trust}"
...
# ---------------------------------------------------------------------------
# Step 2: Read the colocated canonical E4 receipt, or fetch it from docs-site
# ---------------------------------------------------------------------------
echo "[2/5] Loading canonical E4 decision receipt..."
LOCAL_RECEIPT="$SCRIPT_DIR/runs/E4-codex-kernel-2026-04-29/decision_receipt.json"
if [ -f "$LOCAL_RECEIPT" ]; then
    RECEIPT_JSON=$(cat "$LOCAL_RECEIPT")
    echo "      ✓ Receipt loaded from colocated docs-site artifact"
else
    RECEIPT_JSON=$(curl -fsSL \
        "$TRUST_BASE/runs/E4-codex-kernel-2026-04-29/decision_receipt.json" 2>/dev/null) || {
        echo "      ✗ FAILED: Cannot fetch canonical docs-site decision_receipt.json"
        exit 1
    }
    echo "      ✓ Receipt fetched from canonical docs-site source"
fi
